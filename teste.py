...
class UnOp(Node):
    def evaluate(self):
        if self.value == "MINUS":
            print("AQUIII: ", self.children[0].evaluate())
...
class BinOp(Node):
    def evaluate(self):
        if self.value == "PLUS":
            # print(self.children[0].evaluate() , self.children[1].evaluate())
...
class Parser:
    ...
    def parseExpression(tokenizer):
        ...
        print("Retornei do parseTerm com node = ", node.value, node.children)
        ...
        while tokenizer.next.type == "PLUS" or tokenizer.next.type == "MINUS":
            ...
            if tokenizer.next.type == "PLUS":
                ...
                print("É uma soma! node = ", node.value, node.children)
        ...
        if tokenizer.next.type == "INT":
            ...
            print("vou retornar node = ", node.value)
            for i in node.children:
                print(i.value)
            return node

    def parseTerm(tokenizer):
        print("Entrei no Term")
        node = Parser.parseFactor(tokenizer)
        print("Retornei do parseFactor com node = ", node.value, node.children)
    ...
    def parseFactor(tokenizer):
        ...
        if tokenizer.next.type == "INT":
            print("É um número!")
            if tokenizer.next.type == "FACTORIAL":
