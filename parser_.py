...
import sys
from main import PrePro, Write, Node, UnOp, BinOp, IntVal, StringVal, Identifier, FuncCall, VarDec, Block, Assign, Readln, Print, Return, While, If, FuncDec, NoOp, destination_file
...
class Parser:
    ...
    def run(code):
        Write.delete_main_temp_functions()
        ...
        if Parser.tokenizer.position == len(Parser.tokenizer.source) and Parser.tokenizer.next.type == "EOF":
            table = SymbolTable()
            resultado =  root.evaluate(symbolTable=table)
            # print("SymbolTable: ", table.table)
            Write.write_final_output()
            return resultado
        else:
